...
function LongField({ label, name, form, setForm }: { label: string; name: string; form: FormState; setForm: React.Dispatch<React.SetStateAction<FormState>> }) {
  return <div className="space-y-2 sm:col-span-2"><Label htmlFor={`arsenal-${name}`}>{label}</Label><ColoredTextarea id={`arsenal-${name}`} value={String(form[name] ?? "")} onChange={(value) => setForm((current) => ({ ...current, [name]: value }))} /></div>;
}
